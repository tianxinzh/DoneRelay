...
const HELP = `DoneRelay 0.1.0-alpha.1
  ...
  donerelay codex --prompt TEXT [--cwd DIRECTORY]
...
try {
  ...
  if (values.help || !command) console.log(HELP);
    ...
    else if (command === 'codex') r = await runCodex({ prompt: values.prompt, cwd: values.cwd, command: process.env.CODEX_BIN ?? 'codex', client });
