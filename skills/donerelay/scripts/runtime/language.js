...
const labels = {
  en: { notification: 'NOTIFICATION', question: 'QUESTION', approval: 'APPROVAL', task: 'Task', request: 'Request', expires: 'Expires',
    ...
    directQuestion: 'Reply to this message with your answer, or use the numbered command above.', replyPlaceholder: 'Type your answer',
    directApproval: 'You can also reply to this message with approve or deny.',
  ...
  zh: { notification: '通知', question: '问题', approval: '审批', task: '任务', request: '请求', expires: '到期时间',
    ...
    directQuestion: '直接回复本条消息即可回答，也可以使用上面的带编号命令。', replyPlaceholder: '请输入回答',
    directApproval: '也可以直接回复本条消息，输入“批准”或“拒绝”。',
...
export function replyError(error, language) {
  ...
  if (message === 'Reply to the original DoneRelay request message, or send a numbered command without replying.') return '请回复原始请求消息；转发、复制或旧版消息无法自动关联请求。';
  if (message === 'Reply request ID does not match the original message.') return '回复中的请求编号与原始消息不一致。';
  if (message === 'Reply with text to the original request.') return '请使用文字回复原始请求。';
  if (message === 'Reply with approve or deny, or use the buttons.') return '请回复“批准”或“拒绝”，也可以点击按钮。';
  if (message === 'This message does not accept answers or decisions.') return '此消息不接受回答或审批决定。';
