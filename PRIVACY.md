...
Slack uses the existing Codex/Claude MCP connection; DoneRelay stores no Slack OAuth tokens. For questions and approvals, private local `slack.json` records contain the proposal, workspace/user/DM IDs, provider timestamps, answer, workflow lease and single-use consumption state. Finished records are pruned on subsequent commands seven days after expiry. Close workflows before `uninstall --purge` to remove this history. Notification preparation remains stateless. Delete temporary payload/thread files after use. The host and Slack retain tool/chat history under their own policies.
